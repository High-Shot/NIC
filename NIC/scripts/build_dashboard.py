"""
NIC Industries — Dashboard Builder (ORPHANED / manual-only — NOT the live build).

This is a standalone reference script, not the pipeline that generates the
live dashboard. It is NOT wired to any GitHub Actions workflow and does NOT
run on a schedule (there is no workflow in this repo). It must be run manually.

It also does NOT reproduce the live root index.html: it emits its own inline
HTML_TEMPLATE with a `const DATA = {...}` blob and writes a data.json, which is
a different schema from the live dashboard's `var MKT_META` / `var WEEKS`
payload. The live index.html is generated by a Google Apps Script
`exportDashboard()` (in the Google Sheet) that injects data into
NIC/NIC_Dashboard_Template.html — see README.md.

Manual usage (reference only):
    pip install -r requirements.txt
    export GOOGLE_SHEET_ID=<sheet id>
    export GOOGLE_CREDENTIALS_PATH=/path/to/credentials.json
    python NIC/scripts/build_dashboard.py

Pulls data from the Google Sheet tracker and outputs data.json and index.html.
"""
